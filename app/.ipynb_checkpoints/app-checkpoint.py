...
# Create the complete Streamlit dashboard code - fixed version
...
# ============================================================
    ...
    with tab1:
        ...
        rmse_values = [2678, 2425, 1866]
        mae_values = [1959, 1765, 1355]
        ...
        fig.add_trace(
            go.Bar(x=models, y=rmse_values, marker_color=colors, name='RMSE',
                   text=[f'€{v}' for v in rmse_values], textposition='outside'),
            row=1, col=1
        )
        
        fig.add_trace(
            go.Bar(x=models, y=mae_values, marker_color=colors, name='MAE',
                   text=[f'€{v}' for v in mae_values], textposition='outside'),
            row=1, col=2
        )
...
# Save the file
output_path = '/mnt/kimi/output/streamlit_app.py'
with open(output_path, 'w') as f:
    f.write(streamlit_code)

print("✅ Streamlit dashboard saved successfully!")
print(f"📁 Location: {output_path}")
print(f"📊 Size: {len(streamlit_code):,} characters")
print(f"\n🚀 To run:")
print(f"   1. Copy to: app/streamlit_app.py")
print(f"   2. Run: streamlit run app/streamlit_app.py")
